...
Apache Kafka is a **Message Broker** application for Messaging communication, also known as a **distributed commit log** or **distributed streaming platform**.
...
| `config/kraft/server.properties` | Main configuration file |
...
A **Topic** is similar to a table in a database — it is used to store messages sent by producers.

- Data in a topic is stored as a **Log** (append-only, ordered by arrival time)
- This makes writes extremely fast since data is always appended at the end
...
| Key Value | Behavior |
|-----------|----------|
| `null` (default) | Always sent to the same partition |
| Specific key | Routed deterministically based on hash |
| Different keys | May be distributed across different partitions |
...
| NodeJS | https://github.com/tulios/kafkajs |
| Java | Built-in (Kafka is built with Java) |
